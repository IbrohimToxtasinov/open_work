...
class WorkerProfileBloc extends Bloc<WorkerProfileEvent, WorkerProfileState> {
  ...
  Future<void> getWorkerInfo(event, emit) async {
    ...
    if(myResponse.errorMessage.isEmpty){
      emit(state.copyWith(status: FormStatus.gettingWorkerInfoInSuccess,worker: myResponse.data));
    }else{
      emit(state.copyWith(status: FormStatus.gettingWorkerInfoInFailury,errorMessage: myResponse.errorMessage));
    }
  }
